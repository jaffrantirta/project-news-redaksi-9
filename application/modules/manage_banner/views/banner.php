...
<div class="container">
    <h3><span class="glyphicon glyphicon-file"></span>Banner</h3>
    <ol class="breadcrumb">
        <li>List</li>
        <li class="active">Data</li>
        <li><a href="<?php echo base_url() . 'manage_banner/tambahbanner' ?>">Add</a></li>
    </ol>
    <div class="row">
        <div class="col-md-6">
            <?php if (isset($search))
        ...
        <div class="col-md-6">
            <?php echo form_open('manage_banner/'); ?>
    ...
    <table class="table table-striped">
        ...
        if (isset($datas))
        {
            ...
            foreach ($datas as $data)
            {
                echo '<tr>';
                echo '<td>' . $index . '</td>';
                echo '<td><a href="' . base_url() . 'manage_banner/editbanner/' . $data->id . '" class="btn btn-primary">Edit</a>&nbsp;&nbsp;&nbsp;<button onclick="confirmDelete('.$data->id.',\'banner\')" class="btn btn-danger">Delete</button></td>';
                echo '<td>' . $data->start_date . '</td>';
                echo '<td>' . $data->end_date . '</td>';
                echo '<td>' . $data->position . '</td>';
                echo '<td><img class="img-responsive" src="'.base_url().'uploads/banner/'.$data->img.'"></td>';
                if($data->status=="Hide"){
